...
const NewPropertyDiscussion = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      router.push(`/community/property/detailed?id=${listingId}`); // Redirect to detailed discussion page
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-4xl font-bold mb-8 text-center">Start a New Discussion for Listing</h1>

      {error && <p className="text-red-500 text-center mb-4">{error}</p>}

      <form onSubmit={handleSubmit} className="bg-white shadow-lg rounded-lg p-6">
        <div className="mb-6">
          <label htmlFor="content" className="block text-gray-700 font-semibold mb-2">Discussion Content</label>
          <textarea
            id="content"
...
export default NewPropertyDiscussion;
